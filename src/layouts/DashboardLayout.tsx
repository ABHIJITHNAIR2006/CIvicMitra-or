    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        const userDoc = await getDoc(doc(db, "users", user.uid)).catch(e => {
          console.warn("User profile fetch deferred (offline or database initializing):", e?.message || e);
          return null;
        });
        ...
        if (userDoc && userDoc.exists()) {
          ...
          setUserName(userData.fullName || userData.username || user.displayName || "Eco Citizen");
          ...
          if (cached) {
            try {
              ...
              setFirestorePoints(parsed.points || 0);
              setUserName(parsed.fullName || parsed.username || user.displayName || "Eco Citizen");
              if (parsed.role === Role.ADMIN || user.email === "arcadeabhi6@gmail.com") {
                setIsAdmin(true);
              }
            } catch {}
          } else {
            setUserName(user.displayName || user.email?.split("@")[0] || "Eco Citizen");
