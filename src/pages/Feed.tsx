...
export default function Feed() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snap) => {
      ...
      console.warn("Feed completions deferred (offline or database initializing):", error?.message || error);
      setLoading(false);
    });
      ...
      <div className="max-w-2xl mx-auto space-y-8">
        ...
        {loading ? (
          ...
          <div className="space-y-8">
            {posts.map((post, idx) => (
              <PostCard key={post.id ? `${post.id}-${idx}` : `post-${idx}`} post={post} isAdmin={isAdmin} />
            ))}
          </div>
        )}
      </div>
              ...
              <div className="space-y-3 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                {comments.length > 0 ? (
                  comments.map((comment, idx) => (
                    <div key={comment.id ? `${comment.id}-${idx}` : `comment-${idx}`} className="flex gap-2 group/comment">
                      <img 
