...
import { UserProfile, Completion, Role } from "../types";
...
export default function Profile() {
  ...
  // Top 4 badges
  const uniqueEarned = Array.from(
    new Map(userBadges.earned.map(b => [b.id, b])).values()
  );
  const topBadges = uniqueEarned
    .sort((a, b) => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        const userDoc = await getDoc(doc(db, "users", auth.currentUser.uid)).catch(e => {
          console.warn("User profile fetch deferred:", e?.message || e);
          return null;
        });
        if (userDoc && userDoc.exists()) {
          setProfile(userDoc.data() as UserProfile);
        } else {
          const cached = localStorage.getItem(`user_profile_${auth.currentUser.uid}`);
          if (cached) {
            ...
            setProfile({
              ...
              fullName: auth.currentUser.displayName || auth.currentUser.email?.split("@")[0] || "Eco Citizen",
              username: auth.currentUser.email?.split("@")[0] || "citizen",
              points: 0,
              totalPoints: 0,
              currentStreak: 1,
              longestStreak: 1,
        ...
        const compSnap = await getDocs(compQuery).catch(e => {
          console.warn("Completions fetch deferred:", e?.message || e);
          return null;
        });
        if (compSnap) {
          ...
          <div className="min-h-[400px]">
            {activeTab === "ACTIVITY" && (
              <div className="space-y-4">
                {completions.length > 0 ? completions.map((comp, idx) => (
                  <div key={comp.id ? `${comp.id}-${idx}` : `comp-${idx}`} className="bg-card p-6 rounded-3xl card-shadow flex items-center justify-between border border-primary/10">
            ...
            {activeTab === "GALLERY" && (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {completions.map((comp, idx) => (
                  <div key={comp.id ? `${comp.id}-${idx}` : `gallery-${idx}`} className="aspect-square rounded-2xl overflow-hidden card-shadow border border-primary/10">
                    <img src={comp.proofUrl} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                  </div>
                ))}
              </div>
