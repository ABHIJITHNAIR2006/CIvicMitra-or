  ...
  const fetchEvents = useCallback(async () => {
    try {
      ...
      const snap = await getDocs(q).catch(e => {
        console.warn("Events fetch deferred (offline or database initializing):", e?.message || e);
        return null;
      });
      if (snap && !snap.empty) {
      ...
      console.warn("Error fetching events:", error?.message || "Unknown error");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleSeedData = async () => {
    ...
    try {
      const eventsData = [
        ...
        {
          eventId: "solar-rooftop-community-expo",
          title: "Solar Rooftop & Clean Energy Expo",
          description: "Discover residential solar rooftop systems, net-metering subsidies, and home battery setups with certified solar technicians and clean-energy advocates.",
          shortDescription: "Explore solar rooftop setups, subsidies, and clean energy tech.",
          eventType: "EXPO",
          type: "EXPO",
          category: "ENERGY",
          bonusPoints: 45,
          points: 45,
          iconEmoji: "☀️",
          bannerImageUrl: "https://picsum.photos/seed/solarexpo/800/400",
          location: { venueName: "City Exhibition Ground", address: "" },
          ...
          isActive: true,
          isFeatured: true,
          proofInstructions: "Upload a photo from the solar technology exhibition hall.",
          status: "UPCOMING"
        },
        {
          eventId: "zero-waste-repair-cafe",
          title: "Community Zero-Waste Repair Cafe",
          description: "Bring broken small appliances, torn backpacks, bicycles, and wooden items to fix together with volunteer fixers, keeping goods out of landfills.",
          shortDescription: "Fix broken appliances, clothes, and gear with volunteer fixers.",
          eventType: "REPAIR_CAFE",
          type: "REPAIR_CAFE",
          category: "WASTE",
          bonusPoints: 35,
          points: 35,
          iconEmoji: "🛠️",
          bannerImageUrl: "https://picsum.photos/seed/repaircafe/800/400",
          location: { venueName: "Green Civic Maker Hub", address: "" },
          startDate: "2026-11-28T10:00:00Z",
          date: "2026-11-28",
          endDate: "2026-11-28T14:30:00Z",
          maxParticipants: 60,
          registrationDeadline: "2026-11-26",
          isActive: true,
          isFeatured: false,
          proofInstructions: "Upload a photo of your item being repaired at the workstation.",
          status: "UPCOMING"
        },
        {
          eventId: "urban-cyclothon-carfree",
          title: "City Cyclothon & Open Streets Morning",
          description: "Pedal down a scenic 15km car-free morning boulevard route with hundreds of urban cyclists celebrating clean air and sustainable mobility.",
          shortDescription: "Join the 15km community cycling ride on open car-free streets.",
          eventType: "CYCLOTHON",
          type: "CYCLOTHON",
          category: "TRANSPORT",
          bonusPoints: 50,
          points: 50,
          iconEmoji: "🚴",
          bannerImageUrl: "https://picsum.photos/seed/citycyclothon/800/400",
          location: { venueName: "Central Promenade Boulevard", address: "" },
          startDate: "2026-12-06T06:00:00Z",
          date: "2026-12-06",
          endDate: "2026-12-06T09:30:00Z",
          maxParticipants: 300,
          registrationDeadline: "2026-12-03",
          isActive: true,
          isFeatured: true,
          proofInstructions: "Upload a photo with your bicycle at the start line or finish arch.",
          status: "UPCOMING"
        },
      ];
      ...
      <div className="space-y-8">
        ...
        {loading || eventDataLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {loading ? (
                ...
                events.map((event, idx) => (
                  <EventCard 
                    key={event.id ? `${event.id}-${idx}` : `event-${idx}`} 
                    event={event} 
                    isAdmin={isAdmin}
          ...
          <div className="space-y-12">
            {events.map((event, idx) => (
              <div key={event.id ? `${event.id}-${idx}` : `event-group-${idx}`} className="space-y-4">
