...
export default function Leaderboard() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snap) => {
      ...
      console.warn("Leaderboard data deferred (offline or database initializing):", error?.message || error);
      // Fallback to placeholder leaderboard if offline
      if (users.length === 0) {
        setUsers([
          { uid: "seed-1", username: "greenthumb", fullName: "Aarav Sharma", points: 1420, totalPoints: 1420, currentStreak: 12, longestStreak: 15, level: 3, experiencePoints: 1420, role: Role.USER, createdAt: "", email: "" },
          { uid: "seed-2", username: "ecowarrior", fullName: "Priya Patel", points: 1180, totalPoints: 1180, currentStreak: 8, longestStreak: 10, level: 2, experiencePoints: 1180, role: Role.USER, createdAt: "", email: "" },
          { uid: "seed-3", username: "solarkid", fullName: "Rohan Varma", points: 950, totalPoints: 950, currentStreak: 5, longestStreak: 7, level: 2, experiencePoints: 950, role: Role.USER, createdAt: "", email: "" },
        ]);
      }
      setLoading(false);
    });
            ...
            <div className="divide-y divide-primary/5">
              {users.map((user, i) => (
                <motion.div 
                  ...
                  key={user.uid ? `${user.uid}-${i}` : `user-${i}`} 
                  className={cn(
