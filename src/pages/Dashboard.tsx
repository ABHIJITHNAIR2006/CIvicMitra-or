    ...
    try {
      ...
      const [challengesSnap, activitySnap, usersSnap] = await Promise.all([
        getDocs(query(collection(db, "challenges"), where("isDaily", "==", true), limit(3))).catch(e => {
          console.warn("Challenges fetch deferred (offline or database initializing):", e?.message || e);
          return null;
        }),
        getDocs(query(
          ...
          console.warn("Activity fetch deferred (offline or database initializing):", e?.message || e);
          return null;
        }),
        getDocs(collection(db, "users")).catch(e => {
          console.warn("Users fetch deferred (offline or database initializing):", e?.message || e);
          return null;
        })
      ]);
      ...
      const quizSnap = await getDocs(quizQuery).catch(e => {
        console.warn("Quiz check deferred:", e?.message || e);
        return null;
      });
      ...
      console.warn("Dashboard remote data fetch deferred:", error?.message || error);
    } finally {
      setLoading(false);
    }
  };
          ...
          {dailyChallenges.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {dailyChallenges.map((challenge, idx) => (
                <ChallengeCard 
                  key={challenge.challengeId ? `${challenge.challengeId}-${idx}` : `challenge-${idx}`} 
                  challenge={challenge} 
                ...
                {recentActivity.length > 0 ? (
                  <div className="divide-y divide-primary/5">
                    {recentActivity.map((activity, idx) => (
                      <ActivityItem key={activity.id ? `${activity.id}-${idx}` : `activity-${idx}`} activity={activity} />
                    ))}
                  </div>
                ) : (
                  ...
                  <>
                    <div className="divide-y divide-primary/5">
                      {userSubmissions.map((sub, idx) => (
                        <div key={sub.id ? `${sub.id}-${idx}` : `sub-${idx}`} className="p-6 flex items-center justify-between hover:bg-primary/5 transition-colors">
