        ...
        <div className="space-y-4">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 gap-4">
              {questions.map((q, idx) => (
                <div key={q.id ? `${q.id}-${idx}` : `question-${idx}`} className="bg-card p-6 rounded-3xl card-shadow flex justify-between items-start group">
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-x-8 gap-y-1 text-sm text-text-secondary">
                      {q.options.map((opt, i) => (
                        <div key={`opt-${i}`} className={i === q.correctAnswer ? "text-primary font-bold" : ""}>
