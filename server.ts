...
function extractImagePayload(input: string): { mimeType: string; data: string } {
...
async function startServer() {
  ...
  app.post("/api/screen-scan", async (req, res) => {
    try {
      ...
      const ai = getGenAI();
      const { mimeType, data: base64Data } = extractImagePayload(imageBase64);
...
Return a JSON object conforming strictly to this format:
{
  ...
  app.post("/api/verify-proof", async (req, res) => {
    try {
      ...
      const ai = getGenAI();
      const { mimeType, data: base64Data } = extractImagePayload(imageUrl);
